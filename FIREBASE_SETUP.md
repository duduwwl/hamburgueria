...
O checkout agora permite **entrega com taxa calculada pelo CEP** ou **retirada na loja** (sem taxa), e o catálogo troca o antigo prato executivo pelo **Marmitex Completo Na Brasa**. Depois de configurar o ambiente, execute novamente o seed de produtos para refletir essa troca no Firestore.

Para testar a interface da gerência localmente, `outputs/app-config.js` vem com `NABRASA_DEMO_MANAGER_MODE = true`. Esse modo aceita qualquer e-mail e senha preenchidos, mas não acessa pedidos reais. Antes de publicar o site, altere o valor para `false` e use contas autorizadas no Firebase Authentication.
