...
const STATUS_LABELS = Object.freeze({
  ...
  [STATUS.READY_FOR_PICKUP]: "Pronto para retirada",
...
const ALLOWED_STATUS_TRANSITIONS = Object.freeze({
  ...
  [STATUS.PREPARING]: new Set([STATUS.READY_FOR_PICKUP, STATUS.ON_THE_WAY, STATUS.CANCELLED]),
  [STATUS.READY_FOR_PICKUP]: new Set([STATUS.DELIVERED, STATUS.CANCELLED]),
...
function normalizeFulfillment(value) {
  const normalized = String(value ?? "DELIVERY").trim().toUpperCase();
  if (normalized === "DELIVERY" || normalized === "ENTREGA") return "DELIVERY";
  if (normalized === "PICKUP" || normalized === "RETIRADA") return "PICKUP";
  fail("invalid-argument", "Escolha entrega ou retirada na loja.");
}

function pickupQuote() {
  return {
    zone: "retirada-na-loja",
    description: "Retirada na loja — Rua da Brasa, 88, Centro, Lavras, MG",
    feeCents: 0,
    minMinutes: 20,
    maxMinutes: 30
  };
...
function normalizeStatus(value) {
  ...
  const aliases = {
    ...
    PRONTO_PARA_RETIRADA: STATUS.READY_FOR_PICKUP,
...
function normalizeCreateOrderPayload(data) {
  ...
  const fulfillment = normalizeFulfillment(root.fulfillment ?? delivery.method);
  ...
  const cep = isPickup ? "" : normalizeCep(delivery.cep ?? root.cep);
  const street = isPickup ? "Rua da Brasa, 88 — Centro, Lavras, MG" : cleanText(
    delivery.street ?? delivery.address ?? delivery.line1 ?? customer.address ?? root.address,
    "Endereço",
    4,
    180
  );
  const number = isPickup ? "LOJA" : cleanText(delivery.number ?? customer.number ?? root.number ?? "S/N", "Número", 1, 30);
  const complement = isPickup ? "" : cleanText(
  ...
  const reference = isPickup ? "Retirada no balcão" : cleanText(
...
function managerOrderPayload(orderId, order) {
  ...
  return {
    ...
    fulfillment: safeText(order.fulfillment, 20) || "DELIVERY",
  ...
  for (let attempt = 0; attempt < 5; attempt += 1) {
    ...
    const result = await db.runTransaction(async (transaction) => {
      ...
      const order = {
        ...
        delivery: {
          ...normalized.delivery,
          formattedCep: normalized.fulfillment === "PICKUP" ? "RETIRADA" : formatCep(normalized.delivery.cep),
          zone: quote.zone,
          description: quote.description,
          feeCents: quote.feeCents,
          estimatedDeliveryMinutes: { min: quote.minMinutes, max: quote.maxMinutes }
...
exports.createOrder = onCall(callableOptions, async (request) => {
  ...
  const quote = normalized.fulfillment === "PICKUP" ? pickupQuote() : calculateDelivery(normalized.delivery.cep);
